    ...
    <section id="" class="services my-3">
        ...
        <form method="POST" class="grid grid-cols-1 bg-indigo-600 rounded shadow-lg mx-auto p-3 w-3/5 mb-4" action="{{ route('service.update', $service->id) }}" enctype="multipart/form-data">
            ...
            <div class="my-4 col-span-2">
                ...
                <input type="text" class="px-3 py-1 w-full rounded-md" value="{{ $service->description }}" name="description">
            </div>
            <div class="my-4 col-span-2">
                <label class="text-white text-base">Icone du service</label> 
                <select class="custom-select px-3 py-1 w-full rounded-md" value="{{ $service->icon }}" name="icon">
